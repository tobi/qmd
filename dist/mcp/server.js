...
\`\`\``,
        ...
        inputSchema: {
            query: z.string().optional().describe("Plain text query — auto-expanded by the trained model into lex/vec/hyde " +
                "variants, then fused via RRF and reranked. Recommended default for most searches. " +
                "Mutually exclusive with 'searches'."),
            searches: z.array(subSearchSchema).max(10).optional().describe("Typed sub-queries to execute (lex/vec/hyde). First gets 2x weight. " +
            ...
            explain: z.boolean().optional().default(false).describe("Include retrieval score traces (FTS scores, vector scores, RRF fusion breakdown, " +
