...
describe("remote embedding routing", () => {
  ...
  function workingLocalGgufMock() {
    ...
    return {
      module: {
        LlamaLogLevel: { error: "error" },
        resolveModelFile: vi.fn(async () => "/tmp/local-fallback-model.gguf"),
        LlamaChatSession: vi.fn() as any,
        getLlama,
      },
      loadModel,
  ...
  test("falls back to local GGUF (with the canonical remote identity) when every remote endpoint fails", async () => {
    const { module: mockModule, loadModel, getLlama } = workingLocalGgufMock();
    setNodeLlamaCppModuleForTest(mockModule);
    ...
    const llm = new LlamaCpp({
      embedModel: ["http://localhost:1234/v1#test-embed-model", "hf:example/local-fallback/model.gguf"],
    });
    try {
      expect(llm.isRemoteEmbed()).toBe(true);
      // The stored/returned identity stays the shared remote model id, even
      // though the vector actually came from the local GGUF fallback — this
      // is what keeps the fallback vector in the same fingerprint/collection
      // as vectors written while the remote endpoint was healthy.
      expect(llm.embedModelName).toBe("test-embed-model");

      const result = await llm.embed("hello");
      // Float32Array -> number[] loses precision (matches the existing local-embed test above).
      expect(result?.model).toBe("test-embed-model");
      ...
      expect(warnSpy.mock.calls.some(call => String(call[0]).includes("falling back to local model"))).toBe(true);
    } finally {
      fetchSpy.mockRestore();
      warnSpy.mockRestore();
      setNodeLlamaCppModuleForTest(null);
  ...
  test("embedBatch also falls back to local GGUF on total remote failure", async () => {
    const { module: mockModule } = workingLocalGgufMock();
    setNodeLlamaCppModuleForTest(mockModule);

    const fetchSpy = vi.spyOn(globalThis, "fetch").mockRejectedValue(new Error("connection refused"));
    const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});

    // embedBatch's local-GGUF path is gated by `_ciMode` (frozen at construction
    // from process.env.CI) to keep CI hermetic against real model loads. This
    // test exercises that same local path via a mock, so it must construct the
    // instance with CI unset, same as the rest of the suite would see outside CI.
  ...
  test("does NOT attempt a local fallback when no local entry is configured (remote-only stays remote-only)", async () => {
    setNodeLlamaCppModuleForTest(neverCalledNodeLlamaCppMock());
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockRejectedValue(new Error("connection refused"));
    const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});

    // Two remote-only endpoints, no trailing local GGUF entry.
    const llm = new LlamaCpp({
      embedModel: ["http://host-a:1234/v1#test-embed-model", "http://host-b:1234/v1#test-embed-model"],
    });
    try {
      expect(await llm.embed("hello")).toBeNull();
