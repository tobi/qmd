 ...
 */
function buildInstructions(store: Store, defaultCollection?: string): string {
  ...
  if (defaultCollection) {
    const col = status.collections.find(c => c.name === defaultCollection);
    const docCount = col?.documents ?? status.totalDocuments;
    lines.push(`QMD is your local search engine over ${docCount} markdown documents (collection: "${defaultCollection}").`);
  } else {
  ...
  if (status.collections.length > 0) {
    lines.push("");
    if (defaultCollection) {
      lines.push(`Default collection: "${defaultCollection}".`);
    }
...
function createMcpServer(store: Store, collectionOverride?: string): McpServer {
  // Collection scoping: URL path (/mcp/RAMP) > QMD_COLLECTION env var > none
  const defaultCollection = collectionOverride || process.env.QMD_COLLECTION || undefined;

  const server = new McpServer(
    { name: "qmd", version: "0.9.9" },
    { instructions: buildInstructions(store, defaultCollection) },
  );
...
\`\`\``,
    ...
    async ({ searches, limit, minScore, collections }) => {
      ...
      // Use default collections if none specified; URL-scoped collection takes priority
      const effectiveCollections = collections
        ?? (defaultCollection ? [defaultCollection] : getDefaultCollectionNames());
...
export async function startMcpHttpServer(port: number, options?: { quiet?: boolean }): Promise<HttpServerHandle> {
  ...
  async function createSession(collection?: string): Promise<WebStandardStreamableHTTPServerTransport> {
    const transport = new WebStandardStreamableHTTPServerTransport({
      sessionIdGenerator: () => randomUUID(),
      enableJsonResponse: true,
      onsessioninitialized: (sessionId: string) => {
        sessions.set(sessionId, transport);
        const scope = collection ? ` [${collection}]` : "";
        log(`${ts()} session ${sessionId.slice(0, 8)}${scope} created (${sessions.size} active)`);
      },
    });
    const server = createMcpServer(store, collection);
    await server.connect(transport);
    return transport;
  }
  ...
  /**
   * Extract collection name from URL path: /mcp/RAMP → "RAMP", /mcp → undefined.
   */
  function collectionFromPath(pathname: string): string | undefined {
  ...
  async function resolveTransport(
    ...
    if (nodeReq.method === "POST" && body?.method === "initialize") {
      const collection = collectionFromPath(nodeReq.url || "");
      return createSession(collection);
    }
    return null;
  }
