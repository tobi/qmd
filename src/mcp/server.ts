...
\`\`\``,
  ...
  server.registerTool(
    "get",
    {
      ...
      annotations: { readOnlyHint: true, openWorldHint: false },
      inputSchema: {
        path: z.string().describe("File path or docid from search results (e.g., 'pages/meeting.md', '#abc123', or 'pages/meeting.md:100' to start at line 100)"),
    ...
    async ({ path, fromLine, maxLines, lineNumbers }) => {
      ...
      if ("error" in result) {
        let msg = `Document not found: ${path}`;
    ...
    async ({ pattern, maxLines, maxBytes, lineNumbers }) => {
      ...
      for (const result of docs) {
        if (result.skipped) {
          content.push({
            type: "text",
            text: `[SKIPPED: ${result.doc.displayPath} - ${result.skipReason}. Use 'qmd_get' with path="${result.doc.displayPath}" to retrieve.]`,
          });
          continue;
        }
