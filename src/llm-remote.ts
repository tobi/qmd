...
export interface RemoteLLMConfig {
  ...
  timeoutMs?: number;
  /** Embedding model URI/name the remote server uses (label only; falls back to default). */
  embedModel?: string;
  /** Generation model URI/name the remote server uses (label only; falls back to default). */
  generateModel?: string;
  /** Rerank model URI/name the remote server uses (label only; falls back to default). */
  rerankModel?: string;
}
...
export class RemoteLLM implements LLM {
  ...
  constructor(config: RemoteLLMConfig) {
    ...
    // Model names are labels used for DB fingerprinting/keys; the remote server
    // owns the actual models. Fall back to the canonical default URIs.
    this.embedModelName = config.embedModel ?? DEFAULT_EMBED_MODEL_URI;
    this.generateModelName = config.generateModel ?? DEFAULT_GENERATE_MODEL_URI;
    this.rerankModelName = config.rerankModel ?? DEFAULT_RERANK_MODEL_URI;
  }
  ...
  /**
   * Detokenize remotely is not supported — tokenize() returns dummy IDs over the
   * wire, so there is nothing meaningful to reconstruct. Returns an empty string;
   * callers (chunk-by-token truncation) fall back to char-based chunking.
   */
  async detokenize(_tokens: readonly unknown[]): Promise<string> {
    return "";
  }
