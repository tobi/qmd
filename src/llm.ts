...
export interface LLM {
  ...
  /**
   * Tokenize text for chunking/truncation decisions.
   */
  tokenize(text: string): Promise<readonly LlamaToken[]>;

  /**
   * Convert tokens back to text after truncation.
   */
  ...
  /** Resolved generation model name/URI when the backend exposes one. */
  readonly generateModelName?: string;

  /** Resolved reranking model name/URI when the backend exposes one. */
...
/** Get the default LLM instance (creates a local LlamaCpp if none is set). */
...
/** Peek at the default LLM without creating a native instance. */
 ...
 * Call this before process exit to prevent native resource leaks.
 */
