...
Based on [tobi/qmd#77](https://github.com/tobi/qmd/pull/77) by [@sujito00](https://github.com/sujito00) and [@dgilperez](https://github.com/dgilperez).
...
- **Shared reward function**: Uses the same `reward.py` as `finetune/` (symlinked)
...
├── reward.py         # → symlink to ../finetune/reward.py
...
├── evals/
│   └── queries.txt   # → symlink to ../finetune/evals/queries.txt
├── exports/
│   └── *.Modelfile   # Ollama model templates
└── tests/            # Unit tests
```
...
`scripts/mlx_expand.py` runs query expansion as a standalone script:
...
- `QMD_MLX_MODEL` — base model path
- `QMD_MLX_ADAPTER` — LoRA adapter path
- `QMD_MLX_TEMP` — temperature (default: 1.0)

## Credits

- Original QMD: [tobi/qmd](https://github.com/tobi/qmd)
- MLX training PR: [tobi/qmd#77](https://github.com/tobi/qmd/pull/77)
