      ...
      let
        ...
        qmd = pkgs.stdenv.mkDerivation {
          ...
          installPhase = ''
            ...
            # native logs for `qmd mcp`, because stdio is JSON-RPC. `--run`
            # fires before bun starts, so the env is in place before the native
            # binding loads. Metal residency is decided in src/llm.ts, not
            # here — no launcher carries that policy.
