...
- Embeddings can alternatively come from a remote OpenAI-compatible server: set `models.embed` (or `QMD_EMBED_MODEL`) to `http[s]://host:port/v1#model-id`, or a list of such URIs for sequential fallback. See "Remote Embedding" in README.md.
